  ...
  const promises = watchedState.feeds.map((feed) => axios
    ...
    .then((response) => {
      ...
      const newPosts = posts.filter(
        (post) => !existedPostsLinks.includes(post.link),
      );
...
const app = () => {
  ...
  const watchedState = onChange(state, (path, value) => render(path, elements, state, value, i18nInstance));
    ...
    .then(() => {
      ...
      elements.postsContainer.addEventListener('click', (event) => {
        const activePost = watchedState.posts.find(
          (post) => post.id === event.target.dataset.id,
        );
