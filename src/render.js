const setStaticTexts = (elements, i18next) => {
  ...
  elements.modalWindowArticleLink.textContent = i18next.t(
    'modalWindow.readArticleButton',
  );
  elements.modalWindowCloseButton.textContent = i18next.t(
...
const renderErrorMessage = (elements, value, i18next) => {
  elements.inputFeedback.textContent = i18next.t(
    `inputFeedback.errors.${value}`,
  );
};
...
const render = (path, elements, watchedState, value, i18next) => {
  switch (path) {
    case 'inputForm.valid':
      renderValidationResult(elements, value);
      break;
    case 'inputForm.state':
      renderInputFormChanges(elements, value, i18next);
      break;
    case 'error':
      renderErrorMessage(elements, value, i18next);
      break;
    case 'feeds':
      renderFeeds(path, elements, watchedState, i18next);
      break;
    case 'posts':
      renderPosts(path, elements, watchedState, i18next);
      break;
